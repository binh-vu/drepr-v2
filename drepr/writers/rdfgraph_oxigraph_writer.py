...
class RDFGraphWriter(StreamClassWriter):
    def __init__(self, prefixes: dict[str, str]):
        self.g = Store()
        self.written_records: dict[SubjVal, NamedNode | BlankNode] = {}
        self.origin_subj: SubjVal = ""
        self.subj: Optional[NamedNode | BlankNode] = None
        self.buffer: list[tuple[NamedNode, NamedNode | BlankNode | Literal]] = []
    ...
    def begin_record(
        ...
        if is_blank:
            ...
            self.subj = NamedNode(subj)  # type: ignore
            self.subj_has_data = True

        if is_buffered:
            self.buffer = [(RDF_type, NamedNode(class_uri))]
        else:
            self.g.add(Quad(self.subj, RDF_type, NamedNode(class_uri)))
    ...
    def end_record(self):
        ...
        if len(self.buffer) > 0:
            for pred, obj in self.buffer:
                self.g.add(Quad(self.subj, pred, obj))
            self.buffer = []
    ...
    def write_data_property(self, predicate_id: str, value: Any, dtype: Optional[str]):
        ...
        if dtype == DREPR_URI:
            ...
            if dtype is not None:
                dtype = NamedNode(dtype)

            value = Literal(value, datatype=dtype)

        self.subj_has_data = True
        if self.is_buffered:
            self.buffer.append((NamedNode(predicate_id), value))
        else:
            assert self.subj is not None
            self.g.add(Quad(self.subj, NamedNode(predicate_id), value))
    ...
    def write_object_property(
        ...
        self.subj_has_data = True
        if self.is_buffered:
            self.buffer.append((NamedNode(predicate_id), object))
        else:
            assert self.subj is not None
            self.g.add(Quad(self.subj, NamedNode(predicate_id), object))

    def write_to_string(self):
        output = io.BytesIO()
        self.g.dump(output, "text/turtle")
        return output.getvalue().decode()
        # return serialize(self.g, output, "text/turtle")

    def write_to_file(self, filepath):
        self.g.dump(filepath, "text/turtle")
